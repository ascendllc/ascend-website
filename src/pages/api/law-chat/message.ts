...
import { LAW_SYSTEM_PROMPT } from "../../../lib/lawSystemPrompt";
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
	...
	try {
		stream = await getOpenAI().chat.completions.create({
			model: "gpt-5.1",
			messages: [{ role: "system", content: LAW_SYSTEM_PROMPT }, ...trimmedHistory],
			stream: true,
		});
	} catch (err) {
		console.error("OpenAI request failed", err);
		return new Response(
			JSON.stringify({ error: "The assistant is unavailable right now, please try again shortly." }),
			{ status: 502 }
		);
	}
